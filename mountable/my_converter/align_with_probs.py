...
Arguments:
    ...
    probs_pickle_path: string specifying the local filepath to an existing probs pickle file, e.g. from eval_beamsearch_ngram.py. If not supplied, the probs will be calculated here
        Note: supply the same model here as used during the prob generation
    ...
    transcribe_device: None, or a string specifying the device that will be used for generating log-probs (i.e. "transcribing").
        ...
        Note: This setting does not apply when you supply probs_pickle_path.
    ...
    batch_size: int specifying batch size that will be used for generating log-probs and doing Viterbi decoding.
        Note: This setting only applies for Viterbi decoding if probs_pickle_path is set.
...
def main(cfg: AlignmentConfig):
    ...
    for start, end in zip(starts, ends):
        ...
        if all_probs is not  None:
            precalculated_probs = get_relevant_probs_batch(all_probs, start, end)
        ...
        (
            ...
            manifest_lines_batch, model, cfg.additional_ctm_grouping_separator, cfg.align_using_pred_text, precalculated_probs
        )
